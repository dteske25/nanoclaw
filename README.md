...
**New:** Supports [Agent Swarms](https://code.claude.com/docs/en/agent-teams). Spin up teams of agents that collaborate in your chat.
...
Then run `/setup`. Claude Code handles everything: dependencies, Discord bot setup, container build, service configuration.
...
**Skills over features.** If you want to add Slack support, don't create a PR that adds Slack alongside Discord. Instead, contribute a [claude code skill](https://code.claude.com/docs/en/skills) like `/add-slack` that transforms your fork. Users then run `/add-slack` and get clean code that does exactly what they need.
...
- **Main channel** - Your private channel (DMs with the bot) for admin control; every other group is completely isolated
...
From the main channel (DMs), you can manage groups and tasks:
```
...
If you want to add a new channel, don't create a PR that adds it alongside Discord. Instead, contribute a skill file (e.g. `.claude/skills/add-slack/SKILL.md`) that teaches Claude Code how to transform a NanoClaw installation to use that channel.
...
- `/add-slack` - Add Slack as a channel. Should give the user option to replace Discord or add as additional channel. Also should be possible to add it as a control channel (where it can trigger actions) or just a channel that can be used in actions triggered elsewhere
...
- `src/channels/discord.ts` - Discord connection, send/receive
