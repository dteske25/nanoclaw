...
### Adding a New Input Channel (e.g., Slack, WhatsApp, SMS)

Questions to ask:
- Which channel? (Slack, WhatsApp, SMS, etc.)
...
1. Ask: "Should Slack use the same @nano trigger, or a different one?"
2. Ask: "Should Slack messages create separate conversation contexts, or share with Discord groups?"
3. Create `src/channels/slack.ts` implementing the `Channel` interface (see `src/channels/discord.ts`)
