...
|-----|----------|---------|
| **Main app logs** | `logs/nanoclaw.log` | Host-side Discord, routing, container spawning |
...
│   ├── messages/         # Outgoing Discord messages
│   ├── tasks/            # Scheduled task commands
│   ├── current_tasks.json    # Read-only: scheduled tasks visible to this group
│   └── available_groups.json # Read-only: Discord guilds for activation (main only)
...
**IPC file types:**
- `messages/*.json` - Agent writes: outgoing Discord messages
- `tasks/*.json` - Agent writes: task operations (schedule, pause, resume, cancel, refresh_groups)
- `current_tasks.json` - Host writes: read-only snapshot of scheduled tasks
- `available_groups.json` - Host writes: read-only list of Discord guilds (main only)
